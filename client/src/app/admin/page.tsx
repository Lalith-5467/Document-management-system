...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="space-y-7 pb-20 relative font-sans">
      ...
      {/* ── Section Tab Nav ── */}
      <div id="admin-tab-nav" className="flex gap-2 overflow-x-auto scrollbar-none pb-2 border-b border-slate-200/80 dark:border-slate-800">
        {SECTIONS.map(sec => (
          <button
            key={sec.id}
...
function Modal({ title, children, onClose }: any) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-md animate-fade-in font-auth-body">
      <div className="w-full max-w-md rounded-3xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xl p-6 space-y-4 animate-pop-in text-slate-900 dark:text-white">
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
          <h3 className="text-base font-black text-slate-900 dark:text-white font-auth-heading">{title}</h3>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-900 dark:hover:text-white p-1.5 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition cursor-pointer"><X className="w-4 h-4" /></button>
        </div>
        {children}
      </div>
    </div>
