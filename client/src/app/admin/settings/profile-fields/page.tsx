      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 flex items-center gap-2 font-auth-heading">
            <UserCircle className="w-6 h-6 text-blue-500" /> Profile Fields
          </h1>
          <p className="text-sm text-slate-500 mt-1 font-medium">Manage custom fields for user registration and profiles.</p>
        </div>
        <button
          onClick={handleOpenCreate}
          className="flex items-center gap-2 px-5 py-2.5 rounded-2xl bg-gradient-to-r from-blue-500 to-blue-600 text-white font-black text-xs shadow-md shadow-blue-500/20 hover:scale-105 transition"
        >
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                ...
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-500">Field Label</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-500">Internal Name</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-500">Type</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-500">Properties</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-500 text-right">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {fields.map(field => (
                <tr key={field.id} className="hover:bg-slate-50/50 transition group">
                  ...
                  <td className="px-6 py-4">
                    <div className="font-bold text-slate-900 text-sm flex items-center gap-2">
                      ...
                      {['full_name', 'email'].includes(field.name) && (
                        <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-slate-100 text-slate-500 uppercase tracking-wider">System</span>
                      )}
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span className="font-mono text-xs font-bold text-slate-500 bg-slate-100 px-2 py-1 rounded-md">{field.name}</span>
                  </td>
                  <td className="px-6 py-4">
                    <span className="text-xs font-bold text-slate-700 capitalize">{field.type}</span>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex flex-col gap-1">
                      <span className={`text-[10px] font-black uppercase tracking-wider ${field.required ? 'text-rose-500' : 'text-slate-400'}`}>
                        {field.required ? 'Required' : 'Optional'}
                      </span>
                      <span className={`text-[10px] font-black uppercase tracking-wider ${field.is_active ? 'text-emerald-500' : 'text-slate-400'}`}>
          ...
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handleSave}>
              <div className="p-6 space-y-4">
                ...
                <div>
                  <label className="block text-xs font-black uppercase tracking-wider text-slate-700 mb-1.5">Field Label</label>
                  <input
                    type="text"
                    required
                    value={formData.label}
                    onChange={e => setFormData({ ...formData, label: e.target.value })}
                    className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-900 focus:outline-none focus:border-blue-500 transition"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2 sm:col-span-1">
                    <label className="block text-xs font-black uppercase tracking-wider text-slate-700 mb-1.5">Internal Name</label>
                    <input
                      type="text"
                      required
                      value={formData.name}
                      ...
                      className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-mono font-bold text-slate-900 focus:outline-none focus:border-blue-500 transition disabled:opacity-50"
                      placeholder="e.g., company_name"
                    />
                  </div>
                  <div className="col-span-2 sm:col-span-1">
                    <label className="block text-xs font-black uppercase tracking-wider text-slate-700 mb-1.5">Input Type</label>
                    <select
                      value={formData.type}
                      onChange={e => setFormData({ ...formData, type: e.target.value as any })}
                      disabled={['full_name', 'email'].includes(formData.name)}
                      className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-900 focus:outline-none focus:border-blue-500 transition disabled:opacity-50 appearance-none"
                    >
                ...
                <div className="pt-2 flex flex-col gap-3">
                  <label className="flex items-center gap-3 cursor-pointer p-3 rounded-xl border border-slate-200 hover:bg-slate-50 transition">
                    <input 
                      ...
                      disabled={['full_name', 'email'].includes(formData.name)}
                      className="w-4 h-4 text-blue-500 focus:ring-blue-500 border-slate-300 rounded disabled:opacity-50"
                    />
                    <div>
                      <div className="text-sm font-bold text-slate-900">Required Field</div>
                      <div className="text-[10px] text-slate-500 font-medium mt-0.5">Users must provide this information to register or update profile.</div>
                    </div>
                  </label>
                  ...
                  <label className="flex items-center gap-3 cursor-pointer p-3 rounded-xl border border-slate-200 hover:bg-slate-50 transition">
                    ...
                    />
                    <div>
                      <div className="text-sm font-bold text-slate-900">Enable Field</div>
                      <div className="text-[10px] text-slate-500 font-medium mt-0.5">Show this field in registration forms and user profiles.</div>
                    </div>
                  </label>
                </div>
              </div>
              ...
              <div className="p-5 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3">
                ...
                <button
                  type="submit"
                  className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-blue-500 to-blue-600 text-white font-black text-xs shadow-md shadow-blue-500/20 hover:scale-105 transition"
                >
