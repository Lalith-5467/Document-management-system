    ...
    <div className="space-y-6 max-w-7xl mx-auto text-slate-900 dark:text-white font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 font-medium">Configure translation options and language preferences for users.</p>
        </div>
        <button
          onClick={handleOpenCreate}
          className="flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-gradient-to-r from-emerald-500 to-emerald-600 text-white font-black text-xs shadow-md shadow-emerald-500/20 hover:opacity-90 hover:scale-105 transition cursor-pointer"
        >
          ...
          <table className="w-full text-left border-collapse table-fixed">
            <thead>
              <tr className="bg-slate-50 dark:bg-[#0B1120] border-b border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300">
                <th className="w-[35%] px-6 py-4 text-xs font-black uppercase tracking-wider text-slate-700 dark:text-slate-300">Language Name</th>
                <th className="w-[20%] px-6 py-4 text-xs font-black uppercase tracking-wider text-slate-700 dark:text-slate-300">Code</th>
                <th className="w-[25%] px-6 py-4 text-xs font-black uppercase tracking-wider text-slate-700 dark:text-slate-300">Status</th>
                <th className="w-[20%] px-6 py-4 text-xs font-black uppercase tracking-wider text-slate-700 dark:text-slate-300 text-right">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/80">
              {languages.map(lang => (
                <tr key={lang.id} className="group hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className="font-mono text-xs font-bold text-slate-800 dark:text-slate-200 uppercase bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-2.5 py-1 rounded-md">{lang.code}</span>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-1.5 bg-slate-50 dark:bg-slate-800/80 p-1 rounded-xl border border-slate-200/80 dark:border-slate-700 inline-flex">
                      <button
                        ...
                        className="p-1.5 rounded-lg text-slate-500 dark:text-slate-400 hover:text-emerald-600 dark:hover:text-emerald-400 hover:bg-white dark:hover:bg-slate-700 transition cursor-pointer"
                        title="Edit Language"
                      >
      ...
      {/* Modal */}
      {modalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/75 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-md shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden text-slate-900 dark:text-white">
            ...
            <form onSubmit={handleSave}>
              <div className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2 sm:col-span-1">
                    <label className="block text-xs font-extrabold uppercase tracking-wider text-slate-600 dark:text-slate-400 mb-1.5">Language Name</label>
                    <input
                      type="text"
                      required
                      value={formData.name}
                      onChange={e => setFormData({ ...formData, name: e.target.value })}
                      className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-bold text-slate-900 dark:text-white focus:outline-none focus:border-emerald-500 transition"
                      placeholder="e.g., German"
                    />
                  </div>
                  <div className="col-span-2 sm:col-span-1">
                    <label className="block text-xs font-extrabold uppercase tracking-wider text-slate-600 dark:text-slate-400 mb-1.5">Native Name</label>
                    <input
                      type="text"
                      required
                ...
                <div>
                  <label className="block text-xs font-extrabold uppercase tracking-wider text-slate-600 dark:text-slate-400 mb-1.5">Language Code (ISO)</label>
                  <input
                    type="text"
                    required
                    maxLength={5}
                    ...
                    className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-mono font-bold text-slate-900 dark:text-white focus:outline-none focus:border-emerald-500 transition"
                ...
                <div className="pt-2 flex flex-col gap-3">
                  <label className="flex items-center gap-3 cursor-pointer p-3 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 transition">
                    <input 
                      ...
                      className="w-4 h-4 text-emerald-500 focus:ring-emerald-500 border-slate-300 rounded cursor-pointer"
                    />
                    <div>
                      <div className="text-xs font-bold text-slate-900 dark:text-white">Enable Language</div>
                      <div className="text-[10px] text-slate-500 dark:text-slate-400 font-medium mt-0.5">Allow users to select this language in their portal.</div>
                    </div>
                  </label>
                  ...
                  <label className="flex items-center gap-3 cursor-pointer p-3 rounded-xl border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 transition">
                    ...
                    />
                    <div>
                      <div className="text-xs font-bold text-slate-900 dark:text-white">Set as Default</div>
                      <div className="text-[10px] text-slate-500 dark:text-slate-400 font-medium mt-0.5">Used automatically for new users and unauthenticated visitors.</div>
                    </div>
                  </label>
                </div>
              </div>
              ...
              <div className="p-5 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 flex justify-end gap-3">
                ...
                <button
                  type="submit"
                  className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-emerald-600 text-white font-black text-xs shadow-md shadow-emerald-500/20 hover:opacity-90 hover:scale-105 transition cursor-pointer"
                >
