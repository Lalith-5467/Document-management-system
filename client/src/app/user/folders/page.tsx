    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800/80 pb-5">
        <div>
          ...
          <p className="text-xs text-slate-600 dark:text-slate-400">Organize and group your documents inside custom folders.</p>
        </div>

        <button
          onClick={handleOpenCreate}
          className="px-5 py-2.5 text-xs font-bold text-white bg-gradient-to-r from-[#FF6B00] via-[#F97316] to-[#EA580C] hover:brightness-110 rounded-xl shadow-md shadow-orange-500/25 border border-orange-400/30 transition-all flex items-center justify-center gap-2.5 whitespace-nowrap active-press hover:scale-[1.02]"
        >
      ...
      <div className="flex items-center justify-between gap-4 bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800 p-3 rounded-2xl shadow-md">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 text-xs bg-slate-50 dark:bg-[#0b1120] border border-slate-200 dark:border-slate-800 rounded-xl text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-[#FF6B00] transition-all"
          />
        </div>
        <span className="text-xs font-bold text-slate-600 dark:text-slate-400">Total: {filteredFolders.length} Folders</span>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800 rounded-3xl p-12 text-center space-y-4 shadow-md">
          ...
          <p className="text-xs text-slate-600 dark:text-slate-400 font-medium">No folders found matching your query.</p>
          <button
            onClick={handleOpenCreate}
            className="px-4 py-2 text-xs font-bold text-white bg-gradient-to-r from-[#FF6B00] to-[#F97316] hover:brightness-110 rounded-xl shadow-md shadow-orange-500/25 transition"
          >
            Create New Folder
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {filteredFolders.map((folder) => (
            <div
              key={folder.id}
              className="bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-800 hover:border-indigo-500/50 p-5 rounded-2xl space-y-3 transition-all group flex flex-col justify-between shadow-md hover:shadow-xl"
            >
              <div className="space-y-2.5">
                ...
                <div>
                  <h3 className="text-sm font-extrabold text-slate-900 dark:text-white group-hover:text-[#FF6B00] dark:group-hover:text-orange-400 transition-colors truncate">
                    {folder.folder_name}
                  </h3>
              ...
              <div className="pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between">
                ...
                <Link
                  ...
                  className="text-xs font-bold text-[#FF6B00] dark:text-orange-400 hover:text-orange-600 dark:hover:text-orange-300 flex items-center gap-1 hover:underline"
                >
                  Open Folder →
                </Link>
              </div>
          ...
          <div className="bg-white dark:bg-[#111827] rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-5 animate-pop-in text-slate-900 dark:text-white">
            ...
            <form onSubmit={handleCreateSubmit} className="space-y-4 text-xs">
              ...
              <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-[#0b1120] border border-slate-200 dark:border-slate-800 space-y-2">
                ...
                {formInitialFile ? (
                  <div className="flex items-center justify-between p-2 rounded-xl bg-slate-900 border border-indigo-500/40 text-xs text-white">
                  ...
                  <button
                    ...
                    className="w-full py-2.5 px-3 rounded-xl border border-dashed border-slate-700 hover:border-indigo-500 text-slate-400 hover:text-white transition flex items-center justify-center gap-2 text-xs font-semibold"
                  >
      ...
      {isEditOpen && selectedFolder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="bg-white dark:bg-[#111827] rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-5 animate-pop-in text-slate-900 dark:text-white">
            <div className="flex items-center justify-between pb-3 border-b border-slate-200 dark:border-slate-800">
              <h2 className="text-base font-bold text-slate-900 dark:text-white">Edit Folder</h2>
      ...
      {isDeleteOpen && selectedFolder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="bg-white dark:bg-[#111827] rounded-3xl max-w-md w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-5 animate-pop-in text-slate-900 dark:text-white">
            <h2 className="text-base font-bold text-slate-900 dark:text-white">Delete Folder</h2>
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              <button onClick={() => setIsDeleteOpen(false)} className="px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 font-semibold text-xs hover:text-slate-900 dark:hover:text-white">
                Cancel
              </button>
              <button onClick={handleDeleteSubmit} disabled={formSubmitting} className="px-4 py-2 rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-bold text-xs flex items-center gap-2 shadow-lg shadow-rose-600/30">
