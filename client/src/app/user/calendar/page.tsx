      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-slate-200 dark:border-slate-800">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-3">
            <span className="w-9 h-9 rounded-2xl bg-orange-100 dark:bg-orange-950/60 text-[#FF6B00] border border-orange-200 dark:border-orange-900/60 flex items-center justify-center shrink-0">
            ...
            {sortedEvents.length === 0 ? (
              ...
              sortedEvents.map((evt, idx) => {
                ...
                return (
                  <div
                    key={evt.id}
                    className="group bg-white dark:bg-slate-900/80 border border-slate-200/80 dark:border-slate-800/80 rounded-2xl p-4 shadow-sm hover:shadow-[0_8px_30px_rgba(255,107,0,0.1)] hover:border-[#FF6B00]/30 transition-all duration-300 cursor-pointer"
              ...
              {upcomingDeadlines.map(evt => {
                ...
                return (
                  <div
                    key={evt.id}
                    onClick={() => setSelectedEvent(evt)}
                    className="flex items-center justify-between gap-3 p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/60 hover:bg-orange-50 dark:hover:bg-orange-950/20 border border-transparent hover:border-[#FF6B00]/20 transition-all cursor-pointer group"
                  >
