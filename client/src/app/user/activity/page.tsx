...
export default function ActivityHistoryPage() {
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-12">
      {/* Toast Notification */}
      {toastMsg && (
        <div className="fixed bottom-5 right-5 z-50 bg-slate-900 text-white text-xs font-semibold px-4 py-3 rounded-2xl shadow-2xl flex items-center gap-2 animate-in fade-in slide-in-from-bottom-3">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800/80 pb-5">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-semibold text-slate-900 dark:text-white">Activity History</span>
          </div>
          <h1 className="text-2xl font-black text-slate-900 dark:text-white flex items-center gap-2">
            🕒 Activity History
          </h1>
          <p className="text-xs text-slate-600 dark:text-slate-400">Comprehensive audit trail of logins, document uploads, downloads, edits, and deletions</p>
        </div>
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto">
          <button
            ...
            className="px-3.5 py-2 text-xs font-semibold text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/60 hover:bg-rose-100 dark:hover:bg-rose-900/60 border border-rose-200 dark:border-rose-800 rounded-xl transition flex items-center gap-1.5 disabled:opacity-50"
          >
      ...
      <div className="bg-white dark:bg-[#111827] p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-md flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 bg-slate-50 dark:bg-[#0b1120] border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-indigo-500 transition-all"
          />
        </div>
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          ...
          <select
            ...
            className="px-3 py-2 bg-slate-50 dark:bg-[#0b1120] border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-semibold text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500 transition-all"
          >
      ...
      {loading ? (
        <div className="bg-white dark:bg-[#111827] p-12 rounded-3xl border border-slate-200 dark:border-slate-800 text-center space-y-3 shadow-md">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Fetching audit activity history logs...</p>
        </div>
      ) : activities.length === 0 ? (
        <div className="bg-white dark:bg-[#111827] p-12 rounded-3xl border border-slate-200 dark:border-slate-800 text-center space-y-3 shadow-md max-w-md mx-auto">
          ...
          <div>
            <h3 className="font-extrabold text-slate-900 dark:text-white text-base">No Activity Logs Found</h3>
        ...
        <div className="bg-white dark:bg-[#111827] p-6 sm:p-8 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl space-y-6">
          <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
            <h3 className="font-extrabold text-slate-900 dark:text-white text-sm">Activity Timeline Audit Log</h3>
            <span className="text-[11px] font-bold px-3 py-1 rounded-full bg-slate-100 dark:bg-[#0b1120] text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-800 font-mono">
                  ...
                  <div className="bg-slate-50 dark:bg-slate-900/60 group-hover:bg-slate-100 dark:group-hover:bg-slate-800/60 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md border ${badge.bg} ${badge.text}`}>
                          {badge.label}
                        </span>
                        {act.document_name && (
                          <span className="font-bold text-slate-900 dark:text-white text-xs flex items-center gap-1 truncate max-w-xs sm:max-w-md">
                    ...
                    <div className="text-left sm:text-right shrink-0">
                      <div className="text-xs font-bold text-slate-800 dark:text-slate-200">{dt.date}</div>
                      <div className="text-[10px] font-mono text-slate-500 dark:text-slate-400 mt-0.5">{dt.time}</div>
                    </div>
                  </div>
                </div>
              );
          ...
          <div className="bg-white dark:bg-[#111827] w-full max-w-md p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl space-y-5 text-slate-900 dark:text-white">
            ...
            <div className="flex items-center justify-end gap-3 pt-2 border-t border-slate-200 dark:border-slate-800">
              ...
              <button
                ...
                disabled={clearing}
                className="px-4 py-2.5 text-xs font-semibold text-white bg-rose-600 hover:bg-rose-700 rounded-xl shadow-md transition flex items-center gap-1.5 disabled:opacity-60"
              >
                {clearing ? (
                  <>
